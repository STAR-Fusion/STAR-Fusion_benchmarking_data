...
if (file.exists("all.scored.sensitivity_vs_expr.dat.RData") && ! NO_REUSE) {
    print('RESTORING DATA FROM EARLIER ANALYSIS')
    load("all.scored.sensitivity_vs_expr.dat.RData")
} else {
    print('Reading matrix file.')
    primary_data = read.table("all.scored.sensitivity_vs_expr.dat", header=T, com='', sep="\t", row.names=1, check.names=F)
    primary_data = as.matrix(primary_data)
}
source("/ahg/regev/users/bhaas/seq/bhaas/GIT/trinityrnaseq/Analysis/DifferentialExpression/R/heatmap.3.R")
...
write.table(data, file="all.scored.sensitivity_vs_expr.dat.dat", quote=F, sep='	');
if (nrow(data) < 2) { stop("
...
", "all.scored.sensitivity_vs_expr.dat" ) )
dev.off()
